...
const DetailWorkers = () => {
  ...
  const handleDetailWorker = async () => {
    try {
      ...
      const workerData = res.data?.data || res.data; // cek apakah ada `data`
      setDetail(workerData || {});
      console.log(workerData, "<<<< res detail workers");
    } catch (error) {
  ...
  const handleGetPortofolio = async (id) => {
    try {
      ...
      console.log(res.data?.data, "<<<< res portfolio");
    } catch (error) {
      console.log(error.message);
    }
  };
  ...
  const handleGetExperience = async (id) => {
    try {
      ...
      console.log(res.data?.data, "<<<< res experience");
    } catch (error) {
      console.log(error.message);
    }
  };
            ...
            <div className="workerswrapper">
              ...
              <div className="RectangleWhiteRight">
                <div className="titlePortfolio">
                  ...
                  >
                    <h3 className={activeTab === "portfolio" ? "active" : ""}>
                      Portfolio
                    </h3>
                    <hr
                      className={`underline ${
                  ...
                  >
                    <h3 className={activeTab === "experience" ? "active" : ""}>
                      Experience
                    </h3>
                    <hr
                      className={`underline ${
                      ...
                      {portfolio.length > 0 ? (
                        portfolio.map((item) => (
                          <div key={item.id} className="portfolio-content">
                            <img
                              src={item.upload_image || portfoliodefault}
                              alt="Portfolio"
                            />
                        ...
                        experience.map((exp) => (
                          <div key={exp.id} className="experience-content">
                            ...
                            <div className="sub-experience">
                              ...
                              <h2>
                                {exp.month_company} {exp.year_company}
                              </h2>
